class Scheduler
  ...
  def self.decide_analysis_metadata
    ...
    #   metadatas = AnalysisMetadata.find_all({:finished => false})
    # else
    #   # metadatas = AnalysisMetadata.find_all({:finished => false, :processing => false, :instance_id => "", :rest => false})
    #   metadatas = AnalysisMetadata.find_all({:finished => false, :rest => false})
    # end
    ...
    for m_id in metadatas.collect {|m| m.id }.sort
      ...
      #   metadata.instance_id = $w.instance_id
      #   metadata.save
      #   puts "\n$$$ Trying to claim #{metadata.id} $$$"
      #   puts "$$$ Sleeping for #{SLEEP_CONSTANT}s $$$\n"
      #   sleep(SLEEP_CONSTANT)
      #   debugger
